    ...
    stages {
        ...
        stage('SCM') {
           ...
           //     slackSend channel: "#${channel}", color: 'warning', iconEmoji:":exclamation:", message: "Build Started - Job_Name:-${env.JOB_NAME}        //BuildNo:-${env.BUILD_NUMBER}  Branch:-${env.BRANCH_NAME} (<${env.BUILD_URL}|Open>)"
                script {
                    dir('./') {
                    git url: "${scmUrl}",branch: "dev", credentialsId: "${gitCred}"
            ...
            steps {
                script {
                    withDockerRegistry(credentialsId: "bafb2122-8644-4b46-8f0e-3f507bcef8c3", url: '') {
                        ...
                        dockerImage = docker.build(dockerRegistry + ":" + commitId, ".")
                    }
                } // End of Script
            } // End of Steps
        ...
        stage('Deploy App in to Dev') {
            steps {
                script {
                    ...
                    sh """
                        kubectl get node
                        kubectl set image deployment/${dpName} ${podName}=${dockerRegistry}:${commitId} -n ${projectName}-${env.BRANCH_NAME}
